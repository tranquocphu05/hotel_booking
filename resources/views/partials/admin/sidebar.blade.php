<aside id="admin-sidebar"
    ...
    <nav class="space-y-2 text-sm">
        <a href="{{ route('admin.dashboard') }}"
            class="block px-3 py-2 rounded hover:bg-gray-50 {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-50' : '' }}">Dashboard</a>
        <a href="{{ route('admin.loai_phong.index') }}"
            class="block px-3 py-2 rounded hover:bg-gray-50 {{ request()->routeIs('admin.loai_phong.*') ? 'bg-indigo-50' : '' }}">Loại
            phòng</a>
        <a href="{{ route('admin.users.index') }}"
            class="block px-3 py-2 rounded hover:bg-gray-50 {{ request()->routeIs('admin.users.*') ? 'bg-indigo-50' : '' }}">Users</a>
        <a href="{{ route('admin.invoices.index') }}"
            class="block px-3 py-2 rounded hover:bg-gray-50 {{ request()->routeIs('admin.invoices.*') ? 'bg-indigo-50' : '' }}">Hóa
            Đơn</a>
